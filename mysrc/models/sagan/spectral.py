r"""https://github.com/leafinity/SAGAN-tensorflow2.0/blob/master/attention.py"""
...
class SpectralConv2DTranspose(SpectralConv2D, tf.keras.layers.Conv2DTranspose):
    ...
    def __init__(self, filters, kernel_size, power_iterations=1, **kwargs):
        super(SpectralConv2DTranspose, self).__init__(
            filters, kernel_size, power_iterations=1, **kwargs)
    ...
    def call(self, inputs):
        ...
        out_height = tf.keras.utils.conv_utils.deconv_output_length(height,
                                                                    kernel_h,
                                                                    padding=self.padding,
                                                                    output_padding=out_pad_h,
                                                                    stride=stride_h,
                                                                    dilation=self.dilation_rate[0])
        out_width = tf.keras.utils.conv_utils.deconv_output_length(width,
                                                                   kernel_w,
                                                                   padding=self.padding,
        ...
        if self.use_bias:
            outputs = tf.nn.bias_add(
                outputs,
                self.bias,
                data_format=tf.keras.utils.conv_utils.convert_data_format(self.data_format, ndim=4))
